...
node('SPARK_BUILDER_JAVA17') {
    ...
    try{
        ...
        stage('initializeEnv') {
            initializeEnv('dhruva-sip-base', scm: repository)
            sh("printenv")
        ...
        stage('buildAndDeploy') {
            withCredentials([file(credentialsId: 'SETTINGS_FILE', variable: 'settingsFile')]) {
                ...
                sh """
                 env
                 mvn --settings $settingsFile clean deploy -DauthNG.statisticsOutput -Pcheck-code-format
                 """
            ...
            if (!this.isPullRequest()) {
                try {
                    this.withCredentials([this.usernamePassword(credentialsId: 'SONAR_DHRUVA', usernameVariable: 'SONAR_USER', passwordVariable: 'SONAR_TOKEN')]) {
                        this.sh "mvn -B sonar:sonar -Dsonar.login=\${SONAR_TOKEN} -Dsonar.github.repository=${this.env.GITHUB_URL}"
                    }
                } catch (Exception e) {
                ...
                try {
                    this.withCredentials([this.usernamePassword(credentialsId: 'SONAR_DHRUVA', usernameVariable: 'SONAR_USER', passwordVariable: 'SONAR_TOKEN')]) {
                        ...
                        mvn -B sonar:sonar -Dsonar.login=\${SONAR_TOKEN} \
                            -Dsonar.github.repository=${this.env.GITHUB_URL} \
                            -Dsonar.pullrequest.branch=${this.env.CHANGE_BRANCH} \
                            -Dsonar.pullrequest.base=${this.env.CHANGE_TARGET} \
                            -Dsonar.pullrequest.key=${this.env.CHANGE_ID}
                        """
                    }
                } catch (Exception e) {
                    echo "Failed to trigger static analysis ${e}"
                }
            }
        }
        stage('postBuild') {
