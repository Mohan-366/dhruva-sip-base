...
public class ControllerConfigTest {
  ...
  public void testSetRecordRouteInterface() throws ParseException {
    ...
    RecordRoute recordRouteReq =
        getRecordRoute("test1", "test.webex.com", 5060, Transport.TCP.name());
  ...
  public void testSetRecordRouteInterfaceStateless() throws ParseException {
    ...
    RecordRoute rr_in =
        getRecordRoute("dummy", "1.1.1.1", 5061, Transport.TLS.name().toLowerCase());
    RecordRoute rr_in2 =
        getRecordRoute("dummy", "3.3.3.3", 5060, Transport.UDP.name().toLowerCase());
    RecordRoute rr_out =
        getRecordRoute("dummy", "2.2.2.2", 5060, Transport.TCP.name().toLowerCase());
    rrl.add(rr1);
    rrl.add(rr2);
    rrl.add(rr3);
    rrl.add(rr4);
