...
public class FailoverResponseCodeTest {
  ...
  void errorCodeTest() {
    ...
    Mockito.when(dhruvaSIPConfigProperties.getDynamicServerGroups()).thenReturn(dynamicServers);
    ...
    // SG name from dynamic SG
    Assert.assertFalse(failoverResponseCode.isCodeInFailoverCodeSet("cisco.webex.com", 599));
    Assert.assertFalse(failoverResponseCode.isCodeInFailoverCodeSet("cisco.webex.com", 566));
