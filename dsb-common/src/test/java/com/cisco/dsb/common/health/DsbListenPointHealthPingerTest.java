...
public class DsbListenPointHealthPingerTest {
  ...
  @Mock SSLSocketFactory mockedSslSocketFactory;
  ...
  @Test(
      description =
          "test for validating dsb health pinger where we will encounter exception but not related to connection to the listenPoint")
  public void testDsbHealthPingerTCPForOtherExceptions() throws IOException {
    ...
    /* There is  server socket listening for TCP transport, faced some other exceptions not related to connection to the socket. so, while trying to ping, it will not fail, and return a service state as online */
    ServiceHealth pingResult = dsbListenPointHealthPinger.ping();
  ...
  public void testIsListeningTCPNegative() throws IOException {
    ...
    Mockito.when(socketFactory.getSocket(transport, host, port))
  ...
  @Test(
      description =
          "test for validating implementation of if listenpoints are available or not for udp transport")
  ...
  @Test(
      description =
          "test for validating implementation of if listenpoints are available or not for UDP transport negative scenario")
  ...
  public void testIsListeningTlsPositive() throws Exception {
    ...
    Socket mockedClientSocket = mock(Socket.class);
    Mockito.when(
            mockedSslSocketFactory.createSocket(
                testListenPoint.getHostIPAddress(), testListenPoint.getPort()))
        .thenReturn(mockedClientSocket);
