...
public class ProxyStatelessTransaction {
    ...
    private static final int[] Transports_UDP = {
            SipTransportType.UDP, SipTransportType.TCP, SipTransportType.TLS
    };

    private static final int[] Transports_TCP = {
            SipTransportType.TCP, SipTransportType.UDP, SipTransportType.TLS
    };

    private static final int[] Transports_TLS = {
            SipTransportType.TLS, SipTransportType.UDP, SipTransportType.TCP
    };
    ...
    private static final Logger Log =
            DhruvaLoggerFactory.getLogger(ProxyStatelessTransaction.class);
    ...
    public synchronized void proxyTo(
        ...
        try {
            ...
            ProxySendMessage.sendRequest(request,,)
                    .subscribe(
                    req -> {},
                    err -> {
                        // Handle exception
    ...
    protected void forceRequestSource(
            SIPRequest request, int sourcePort, InetAddress sourceAddress) {
    ...
    public synchronized void addProxyRecordRoute(
            SIPRequest request, ProxyBranchParamsInterface params) {
    ...
    protected synchronized void prepareRequest(
        ...
        if (processVia()) {
            ...
            int viaTransport = SipTransportType.UDP;
            ...
            if (route == null) viaTransport = destTransport.getValue();
                ...
                if (route.getAddress().getURI().isSipURI()) {
                    ...
                    if(routeURI.getTransportParam() != null) {
                        ...
                        viaTransport = ParseProxyParamUtil.getNetworkTransport(request.getNetwork()).getValue();
                    }
                }
            }
            ...
            via = JainSipHelper.getHeaderFactory().createViaHeader(com.cisco.dhruva.sip.hostPort.HostPortUtil.convertLocalIpToHostInfo(listenIf),listenIf.getPort(), Transport.valueOf(viaTransport).toString(), branch );
...
//            }
    ...
    */
    ...
    protected void addRecordRoute(
            SIPRequest request, URI _requestURI, ProxyBranchParamsInterface params) throws SipException, ParseException {
