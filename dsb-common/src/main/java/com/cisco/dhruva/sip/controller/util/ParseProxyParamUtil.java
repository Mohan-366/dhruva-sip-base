...
public class ParseProxyParamUtil {
        ...
        return () -> {
            try {
                return getParsedProxyParams(
                        request, ReConstants.MY_URI, false, ReConstants.DELIMITER_STR);
    ...
    public static Map<String, String> getParsedProxyParams(
        ...
        switch (type) {
            ...
            case ReConstants.P_A_ID:
                header =
                        (HeaderAddress) request.getHeader(PAssertedIdentityHeader.NAME);
                if (header != null) {
    ...
    public static Transport getNetworkTransport(DhruvaNetwork network) {
        ...
        for (Transport transport : Transports) {
            if (network != null
                    && controllerConfig.getInterface(transport, network) != null) {
