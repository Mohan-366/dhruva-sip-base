...
import com.cisco.dsb.proxy.util.log.event.Event.ErrorType;
import com.cisco.dsb.proxy.util.log.event.Event.EventSubType;
import com.cisco.dsb.proxy.util.log.event.Event.EventType;
...
public class DhruvaLogger implements Logger {
  ...
  /**
   * Uses Apache {@link ExceptionUtils} to provide the Exception class name and message of the
   * throwable, along with the same for all of the exceptions in the "caused by" exception chain and
   * the first line of the stack traces.
   *
   * @param throwable
   * @return
   */
